...
const REDIRECT_URI = 'https://pmabodpaoannaajaakeckdjjoeejfgni.chromiumapp.org/';
...
// Remove or comment out the existing alarm setup
...
// Add an event listener to detect when a tab's audible status changes
...
// Optionally, listen for tab removal to handle cases where an audible tab is closed
...
async function startPlayback() {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error resuming playback:', errorData.error.message || 'Unknown error occurred');
      return;
    }
...
async function isPlaybackPaused(accessToken) {
  const response = await fetch('https://api.spotify.com/v1/me/player', {
    headers: { 'Authorization': `Bearer ${accessToken}` }
  });
  if (!response.ok) return false;
  const data = await response.json();
  return data && data.is_playing === false;
}
